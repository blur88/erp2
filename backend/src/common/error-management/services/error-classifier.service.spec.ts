...
describe("ErrorClassifierService", () => {
  ...
  describe("getConstraintType", () => {
    ...
    it('returns "foreign" for foreign key message', () => {
      expect(service.getConstraintType("violates foreign key constraint")).toBe(
        "foreign",
      );
    });
    ...
    it('returns "check" for check constraint message', () => {
      expect(service.getConstraintType("violates check constraint")).toBe(
        "check",
      );
    });

    it('returns "unknown" for unrecognised message', () => {
      expect(service.getConstraintType("something completely different")).toBe(
        "unknown",
      );
    });
  });
