...
describe('ErrorSanitizerService', () => {
  ...
  describe('sanitizeUserAgent', () => {
    it('replaces three-part version numbers', () => {
      ...
      expect(service.sanitizeUserAgent(ua)).toBe('Mozilla/5.0 AppleWebKit/x.x.x Chrome/x.x.x');
    });
  ...
  describe('sanitizeErrorMessage', () => {
    it('redacts auth credential fields', () => {
      expect(service.sanitizeErrorMessage('password=secret123')).toBe('password=[REDACTED]');
    });

    it('redacts DB table names', () => {
      expect(service.sanitizeErrorMessage('table "users" violates')).toBe('table "[TABLE]" violates');
    });

    it('redacts DB column names', () => {
      expect(service.sanitizeErrorMessage('column "email" of')).toBe('column "[COLUMN]" of');
    });

    it('redacts DB constraint names', () => {
      expect(service.sanitizeErrorMessage('constraint "uq_email" violated')).toBe(
        'constraint "[CONSTRAINT]" violated',
      );
    });

    it('returns [INVALID_MESSAGE] for non-string input', () => {
      expect(service.sanitizeErrorMessage(null as any)).toBe('[INVALID_MESSAGE]');
    });

    it('returns [MESSAGE_TOO_LONG] for messages over 5000 chars', () => {
      expect(service.sanitizeErrorMessage('a'.repeat(5001))).toBe('[MESSAGE_TOO_LONG]');
    });
  });
