...
export class HttpExceptionFilter implements ExceptionFilter {
  constructor(
    ...
    if (exception instanceof HttpException) {
      ...
      ({ status, message, error, errorCode, requestId } = this.handleDatabase(
        exception,
        isProduction,
        request,
      ));
    } else {
      ({ status, message, error, requestId } =
        this.handleUnexpected(isProduction));
    ...
    if (
      ...
      (status !== HttpStatus.BAD_REQUEST || shouldLogBadRequest(exception))
    ) {
      this.errorLogger.logApplicationError(
        status,
        message,
        request,
        requestId,
  ...
  private handleDatabase(
    ...
    const constraintType = this.errorClassifier.getConstraintType(
      rawMessage.toLowerCase(),
    );
    const dbError = this.errorClassifier.getErrorResponse(
      constraintType,
      isProduction,
    );
  ...
  private handleUnexpected(isProduction: boolean): {
    status: number;
    message: string;
    error: string;
    requestId: string;
