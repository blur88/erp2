...
import { DatabaseErrorCode, DatabaseErrorResponse } from './error-types';
...
export class ErrorClassifierService {
  ...
  /**
   * Get error response for constraint type
   */
  getErrorResponse(constraintType: string, isProduction: boolean): DatabaseErrorResponse {
    return this.CONSTRAINT_MESSAGES.get(constraintType) || {
   ...
   */
  getGenericError(isProduction: boolean, requestId?: string): DatabaseErrorResponse {
