...
export class PurchaseOrder extends BaseEntity {
  ...
  calculateTotals(): void {
    ...
    if (
      this.items &&
      this.items.length > 0 &&
      this.items[0].totalAmount !== undefined &&
      this.items[0].totalAmount !== 0
    ) {
    ...
    if (this.discountPercent > 0) {
      this.discountAmount =
        (Number(this.subtotal || 0) * Number(this.discountPercent)) / 100;
    }

    // Calculate total (subtotal - discount + shipping)
    const subtotalAfterDiscount =
      Number(this.subtotal || 0) - Number(this.discountAmount || 0);
