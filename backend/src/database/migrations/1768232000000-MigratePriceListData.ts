...
export class MigratePriceListData1768232000000 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        for (let i = 0; i < pricingSchemes.length; i++) {
            ...
            if (existingPriceList.length > 0) {
                ...
                console.log(`Created price list: ${scheme.name} (${code}) - ID: ${priceListId}`);
            }
        ...
        console.log(`Found ${products.length} products with pricing tiers to migrate`);
        ...
        for (const product of products) {
            if (!product.pricingTiers || typeof product.pricingTiers !== 'object') {
                console.log(`Skipping product ${product.id}: invalid pricingTiers format`);
                itemsSkipped++;
                continue;
            }
            ...
            for (const [schemeName, price] of Object.entries(product.pricingTiers)) {
                ...
                if (!priceListId) {
                    console.log(`Warning: No price list found for scheme "${schemeName}", skipping`);
                    continue;
                }
                ...
                if (isNaN(priceValue) || priceValue < 0) {
                    console.log(`Skipping invalid price for product ${product.id}, scheme ${schemeName}: ${price}`);
                    continue;
                }
                ...
                if (existingItem.length > 0) {
                    console.log(`Price list item already exists for product ${product.id}, scheme ${schemeName}`);
                    itemsSkipped++;
                    continue;
                }
        ...
        console.log(`Created ${itemsCreated} price list items, skipped ${itemsSkipped}`);
        ...
        console.log(`Found ${customers.length} customers with pricing schemes to migrate`);
        ...
        for (const customer of customers) {
            ...
            if (!priceListId) {
                console.log(`Warning: No price list found for customer ${customer.id} scheme "${customer.pricingScheme}"`);
                customersSkipped++;
                continue;
            }
        ...
        console.log(`Updated ${customersUpdated} customers, skipped ${customersSkipped}`);

        // Step 4: Validation summary
        console.log('Step 4: Generating validation summary...');

        const priceListCount = await queryRunner.query(`SELECT COUNT(*) as count FROM "price_lists" WHERE "isActive" = true`);
        const priceListItemCount = await queryRunner.query(`SELECT COUNT(*) as count FROM "price_list_items" WHERE "isActive" = true`);
        const customersWithPriceList = await queryRunner.query(`SELECT COUNT(*) as count FROM "customers" WHERE "priceListId" IS NOT NULL AND "deletedAt" IS NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        console.log(`Restored pricing schemes for ${customersWithPriceList.length} customers`);
