...
describe("FundTransferService", () => {
  ...
  describe("create", () => {
    ...
    it("throws BadRequestException when amount is zero or negative", async () => {
      ...
      await expect(
        service.create({ ...dto, amount: 0 }, "user-1"),
      ).rejects.toThrow(BadRequestException);
    });
  ...
  describe("post", () => {
    ...
    it("posts a draft transfer and links journal entry", async () => {
      transferRepository.findOne.mockResolvedValueOnce(
        mockDraftTransfer as any,
      );
      accountingService.postFundTransferEntry.mockResolvedValue({
        id: "je-1",
      } as any);
    ...
    it("throws BadRequestException when transfer is POSTED", async () => {
      ...
      await expect(service.post("trf-1", "user-1")).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe("unpost", () => {
    ...
    it("throws BadRequestException when transfer is not POSTED", async () => {
      ...
      await expect(service.unpost("trf-1", "user-1")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("remove", () => {
    ...
    it("throws BadRequestException when trying to delete a POSTED transfer", async () => {
      ...
      await expect(service.remove("trf-1", "user-1")).rejects.toThrow(
        BadRequestException,
      );
    });
