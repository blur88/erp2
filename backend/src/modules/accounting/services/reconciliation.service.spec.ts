...
describe("ReconciliationService", () => {
  ...
  const createMockQueryBuilder = (result: any = [], count: number = 0) => ({
    ...
    getRawOne: jest
      .fn()
      .mockResolvedValue({ totalDebit: "50000", totalCredit: "0" }),
  });
  ...
  describe("restore", () => {
    ...
    it("throws BadRequestException when reconciliation is not deleted", async () => {
      ...
      await expect(service.restore("recon-001")).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe("permanentDelete", () => {
    it("permanently deletes a reconciliation", async () => {
      ...
      await expect(
        service.permanentDelete("recon-001", "user-1", "admin"),
    ...
    it("throws NotFoundException when id does not exist", async () => {
      ...
      await expect(service.permanentDelete("bad-id")).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it("throws BadRequestException when reconciliation is not soft-deleted", async () => {
      ...
      await expect(service.permanentDelete("recon-001")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("update", () => {
    ...
    describe("account change", () => {
      ...
      it("deletes all transactions, reloads for new account, resets statementBalance to 0", async () => {
        ...
        expect(reconciledTxnRepo.delete).toHaveBeenCalledWith({
          reconciliationId: "recon-001",
        });
      ...
      it("does not delete or reload transactions when accountId is unchanged (no-op)", async () => {
        ...
        await service.update("recon-001", {
          accountId: "acct-001",
          statementBalance: 55000,
        } as any);
  ...
  describe("remove", () => {
    ...
    it("should throw BadRequestException if reconciliation is completed", async () => {
      ...
      await expect(service.remove("recon-001")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("markCleared", () => {
    it("should mark transactions as cleared", async () => {
      ...
      reconciledTxnRepo.save.mockResolvedValue({
        ...mockTxn,
        cleared: true,
      } as any);
      reconciledTxnRepo.createQueryBuilder.mockReturnValue(
  ...
  describe("complete", () => {
    ...
    it("should throw BadRequestException if already completed", async () => {
      ...
      await expect(service.complete("recon-001")).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe("reopen", () => {
    ...
    it("should throw BadRequestException if not completed", async () => {
      ...
      await expect(service.reopen("recon-001")).rejects.toThrow(
        BadRequestException,
      );
    });
  });
