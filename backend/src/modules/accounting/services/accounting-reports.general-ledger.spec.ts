...
describe("AccountingReportsService - General Ledger", () => {
  ...
  describe("generateGeneralLedger", () => {
    it("should generate general ledger for an account with correct running balance", async () => {
      ...
      queryHelper.queryTransactionTotals.mockResolvedValueOnce(
        new Map([[accountId, { totalDebit: 50000, totalCredit: 0 }]]),
      );
  ...
  describe("exportGeneralLedgerToExcel", () => {
    it("should generate Excel buffer for general ledger report", async () => {
      ...
      const buffer =
        await service.exportGeneralLedgerToExcel(mockGeneralLedger);

      expect(buffer).toBeDefined();
      expect(buffer).toBeInstanceOf(Buffer);
      expect(buffer.length).toBeGreaterThan(0);
      expect(
        excelExportService.exportGeneralLedgerToExcel,
      ).toHaveBeenCalledWith(mockGeneralLedger, "general-ledger");
    });
