...
describe('ExpenseService', () => {
  ...
  it('findAll returns paginated results with filters', async () => {
    ...
    const result = await service.findAll({ page: 1, limit: 20, search: 'util' });
  ...
  it('update updates a draft expense', async () => {
    ...
    const result = await service.update('ex-1', { amount: 75, expenseDate: '2026-02-02' });
  ...
  it('update throws BadRequestException for posted expense', async () => {
    ...
    await expect(service.update('ex-1', { amount: 75 })).rejects.toThrow(BadRequestException);
  });
