...
describe("JournalEntryService", () => {
  ...
  describe("create", () => {
    ...
    it("should create a journal entry with DRAFT status", async () => {
      ...
      journalEntryRepository.create.mockReturnValue(
        mockJournalEntry as JournalEntry,
      );
      journalEntryRepository.save.mockResolvedValue(
        mockJournalEntry as JournalEntry,
      );
      journalEntryLineRepository.create.mockReturnValue(
        mockJournalEntryLine1 as JournalEntryLine,
      );
      ...
      journalEntryRepository.findOne.mockResolvedValueOnce(
        entryWithLines as JournalEntry,
      );
    ...
    it("should throw BadRequestException if less than 2 lines", async () => {
      ...
      chartOfAccountRepository.findOne.mockResolvedValue(
        mockAccount1 as ChartOfAccount,
      );
    ...
    it("should throw ConflictException if reference number already exists", async () => {
      ...
      await expect(service.create(dtoWithRef)).rejects.toThrow(
        ConflictException,
      );
    });
  });
  ...
  describe("findOne", () => {
    it("should return a journal entry by ID", async () => {
      ...
      journalEntryRepository.findOne.mockResolvedValue(
        entryWithRelations as JournalEntry,
      );
  ...
  describe("update", () => {
    ...
    it("should throw BadRequestException if entry is not DRAFT", async () => {
      ...
      journalEntryRepository.findOne.mockResolvedValue(
        postedEntry as JournalEntry,
      );

      await expect(service.update("entry-1", updateDto)).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe("postEntry", () => {
    ...
    it("should throw BadRequestException if entry is not balanced", async () => {
      ...
      journalEntryRepository.findOne.mockResolvedValue(
        unbalancedEntry as JournalEntry,
      );
    ...
    it("should return failures for non-draft entries", async () => {
      ...
      jest
        ...
        .mockRejectedValueOnce(
          new BadRequestException("Cannot delete posted entry"),
        );
  ...
  describe("reverseEntry", () => {
    ...
    it("should throw BadRequestException if entry is not POSTED", async () => {
      ...
      await expect(service.reverseEntry("entry-1")).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it("should throw NotFoundException if entry not found", async () => {
      ...
      await expect(service.reverseEntry("non-existent")).rejects.toThrow(
        NotFoundException,
      );
    });
  });
