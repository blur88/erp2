...
describe("AccountingReportsService - Profit And Loss", () => {
  ...
  describe("exportProfitAndLossToExcel", () => {
    it("should generate Excel buffer for profit and loss report", async () => {
      ...
      const buffer =
        await service.exportProfitAndLossToExcel(mockProfitAndLoss);

      expect(buffer).toBeDefined();
      expect(buffer).toBeInstanceOf(Buffer);
      expect(buffer.length).toBeGreaterThan(0);
      expect(
        excelExportService.exportProfitAndLossToExcel,
      ).toHaveBeenCalledWith(mockProfitAndLoss, "profit-and-loss");
    });
    ...
    it("should handle negative net income (loss)", async () => {
      ...
      expect(
        excelExportService.exportProfitAndLossToExcel,
      ).toHaveBeenCalledWith(mockLoss, "profit-and-loss");
    });
  });
});
