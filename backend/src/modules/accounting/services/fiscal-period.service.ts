...
export class FiscalPeriodService {
  ...
  async restore(id: string, userId?: string, username?: string): Promise<FiscalPeriodResponseDto> {
    ...
    if (!period.deletedAt) {
      throw new BadRequestException(`Fiscal period '${period.name}' is not deleted`);
    }
  ...
  async generateFiscalPeriods(
    ...
    this.logger.log(`Generating fiscal periods for year ${year}, starting from month ${startMonth}`);
    ...
    for (let i = 0; i < 12; i++) {
      ...
      try {
        ...
        if (error instanceof ConflictException) {
          ...
          this.logger.error(`Failed to create period ${code}: ${error.message}`);
          throw error;
        }
      }
    }

    this.logger.log(`Generated ${createdPeriods.length} fiscal periods for year ${year}`);
  ...
  async getCurrentPeriod(): Promise<FiscalPeriodResponseDto | null> {
    ...
    if (!period) {
      this.logger.warn(`No current open period found for date: ${today.toISOString()}`);
      return null;
    }
   ...
   */
  async validatePeriod(dto: ValidatePeriodDto): Promise<FiscalPeriodValidationResponseDto> {
    const { date } = dto;
