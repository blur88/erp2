...
describe("AccountingService", () => {
  ...
  beforeEach(async () => {
    ...
    journalEntryService = module.get(
      JournalEntryService,
    ) as jest.Mocked<JournalEntryService>;
    accountMappingService = module.get(
      AccountMappingService,
    ) as jest.Mocked<AccountMappingService>;
    fiscalPeriodService = module.get(
      FiscalPeriodService,
    ) as jest.Mocked<FiscalPeriodService>;
  });
  ...
  describe("postSalesOrderEntry", () => {
    ...
    it("should create two separate entries: COGS first, then Revenue", async () => {
      ...
      const result = await service.postSalesOrderEntry(
        mockSalesOrder,
        "user-123",
      );
      ...
      // Both entries posted
      expect(journalEntryService.postEntry).toHaveBeenCalledWith(
        "cogs-entry",
        "user-123",
      );
      expect(journalEntryService.postEntry).toHaveBeenCalledWith(
    ...
    it("should calculate COGS correctly", async () => {
      ...
      const cogsLine = cogsCreateCall.lines.find(
        (l: any) => l.accountId === "cogs-account-id",
      );
    ...
    it("should use items sum for AR/Revenue, not stale totalAmount column", async () => {
      ...
      const arLine = revenueCreateCall.lines.find(
        (l: any) => l.accountId === "ar-account-id",
      );
    ...
    it("should throw BadRequestException when items relation is not loaded", async () => {
      ...
      await expect(
        service.postSalesOrderEntry(orderWithoutItems, "user-123"),
      ).rejects.toThrow(
    ...
    it("should throw when items not loaded even when shippingAmount is non-zero", async () => {
      ...
      await expect(
        service.postSalesOrderEntry(orderWithShippingNoItems, "user-123"),
      ).rejects.toThrow(
    ...
    it("creates only the Revenue entry when COGS is zero (service order)", async () => {
      ...
      expect(createCall.description).toBe(
        "Sales Order SO-SVC - Test Customer (Revenue)",
      );
  ...
  describe("postCustomerPaymentEntry", () => {
    ...
    it("should create journal entry with correct lines", async () => {
      const result = await service.postCustomerPaymentEntry(
        mockPayment,
        "user-123",
      );
  ...
  describe("postGoodsReceivedEntry", () => {
    ...
    it("should calculate total correctly from GRN items", async () => {
      ...
      const inventoryLine = createCall.lines.find(
        (l: any) => l.accountId === "purchase-inventory-id",
      );
  ...
  describe("postVendorPaymentEntry", () => {
    ...
    it("should create journal entry with correct lines", async () => {
      const result = await service.postVendorPaymentEntry(
        mockVendorPayment,
        "user-123",
      );
  ...
  describe("postStockAdjustmentEntry", () => {
    ...
    it("should create entry for increase adjustments", async () => {
      const result = await service.postStockAdjustmentEntry(
        mockAdjustmentIncrease,
        "user-123",
      );
    ...
    it("should calculate totals correctly", async () => {
      await service.postStockAdjustmentEntry(
        mockAdjustmentIncrease,
        "user-123",
      );
  ...
  describe("postOpeningBalances", () => {
    ...
    it("should reject if no open fiscal period exists for the date", async () => {
      ...
      await expect(service.postOpeningBalances(dto)).rejects.toThrow(
        BadRequestException,
      );
    });
