...
export class ReconciliationController {
  ...
  @ApiResponse({
    status: 200,
    description: "Returns all deleted bank reconciliations",
  })
  ...
  async bulkRestore(
    ...
    return this.reconciliationService.bulkRestore(
      body.ids,
      currentUserId,
      currentUsername,
    );
  ...
  async bulkPermanentDelete(
    ...
    return this.reconciliationService.bulkPermanentDelete(
      body.ids,
      currentUserId,
      currentUsername,
    );
  ...
  @ApiResponse({
    status: 201,
    description:
      "Bank reconciliation created with unreconciled transactions loaded",
    type: BankReconciliationResponseDto,
  })
  @ApiResponse({
    status: 400,
  ...
  @ApiResponse({
    status: 404,
    description: "Account or fiscal period not found",
  })
  async create(
  ...
  @Auth(UserRole.ADMIN, UserRole.MANAGER)
  @ApiOperation({
    summary: "Update bank reconciliation (statement balance, date)",
  })
  @ApiParam({ name: "id", description: "Bank reconciliation ID" })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 400,
    description: "Cannot update completed reconciliation",
  })
  @ApiResponse({ status: 404, description: "Bank reconciliation not found" })
  async update(
    @Param("id") id: string,
  ...
  @ApiResponse({
    status: 400,
    description: "Not deleted, or duplicate IN_PROGRESS exists",
  })
  @ApiResponse({ status: 404, description: "Bank reconciliation not found" })
  async restore(
    @Param("id") id: string,
    ...
    return this.reconciliationService.restore(
      id,
      currentUserId,
      currentUsername,
    );
  ...
  @ApiResponse({
    status: 204,
    description: "Bank reconciliation permanently deleted",
  })
  @ApiResponse({ status: 404, description: "Bank reconciliation not found" })
  async permanentDelete(
    ...
    await this.reconciliationService.permanentDelete(
      id,
      currentUserId,
      currentUsername,
    );
  ...
  @ApiResponse({
    status: 400,
    description: "Cannot modify completed reconciliation",
  })
  async markCleared(
    @Param("id") id: string,
  ...
  async unmarkCleared(
    ...
    return this.reconciliationService.unmarkCleared(
      id,
      dto,
      currentUserId,
      currentUsername,
  ...
  async reopen(
    ...
    return this.reconciliationService.reopen(
      id,
      currentUserId,
      currentUsername,
    );
