...
export class ChartOfAccountsController {
  ...
  @ApiResponse({
    status: 400,
    description: "Invalid account data or parent type mismatch",
  })
  @ApiResponse({ status: 404, description: "Parent account not found" })
  ...
  @ApiResponse({
    status: 400,
    description:
      "Invalid update data, circular reference, or parent type mismatch",
  })
  @ApiResponse({ status: 404, description: "Account not found" })
  @ApiResponse({ status: 409, description: "Account code already exists" })
  async update(
  ...
  async remove(
    ...
    await this.chartOfAccountsService.remove(
      id,
      currentUserId,
      currentUsername,
    );
  ...
  @ApiResponse({
    status: 400,
    description:
      "Account has children, journal entry lines, or is not soft-deleted",
  })
  @ApiResponse({ status: 404, description: "Account not found" })
  async permanentDelete(
    ...
    await this.chartOfAccountsService.permanentDelete(
      id,
      currentUserId,
      currentUsername,
    );
  ...
  @ApiResponse({
    status: 409,
    description: "Account code now used by another account",
  })
  async restore(
    @Param("id") id: string,
    @CurrentUser("userId") currentUserId: string,
    ...
    return this.chartOfAccountsService.restore(
      id,
      currentUserId,
      currentUsername,
    );
