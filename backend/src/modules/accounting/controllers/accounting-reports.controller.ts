...
export class AccountingReportsController {
  ...
  @ApiOperation({
    ...
    description: 'Lists all accounts with their debit/credit balances as of a specific date. Total Debits must equal Total Credits in a balanced system.',
  })
  @ApiQuery({
    name: 'asOfDate',
    required: false,
    type: String,
    description: 'Calculate trial balance as of this date (ISO 8601 format, defaults to current date)',
    example: '2026-02-10',
  })
  @ApiQuery({
  ...
  async exportTrialBalance(
    ...
    const buffer = await this.accountingReportsService.exportTrialBalanceToExcel(
      data,
      'trial-balance',
    );
  ...
  @ApiOperation({
    ...
    description: 'Lists assets, liabilities, and equity as of a specific date. Follows the equation: Assets = Liabilities + Equity',
  })
  @ApiQuery({
    name: 'asOfDate',
    required: false,
    type: String,
    description: 'Calculate balance sheet as of this date (ISO 8601 format, defaults to current date)',
    example: '2026-02-10',
  })
  @ApiQuery({
    name: 'includeInactive',
  ...
  async exportBalanceSheet(
    ...
    const buffer = await this.accountingReportsService.exportBalanceSheetToExcel(
      data,
      'balance-sheet',
    );
  ...
  @ApiOperation({
    ...
    description: 'Shows revenues, costs, and expenses over a period of time. Calculates Net Income = Revenue - COGS - Expenses',
  })
  @ApiQuery({
    name: 'startDate',
    required: true,
  ...
  @ApiOperation({
    ...
    description: 'Downloads Profit and Loss (Income Statement) report as an Excel file',
  })
  @ApiQuery({
    name: 'startDate',
    required: true,
  ...
  async exportProfitAndLoss(
    ...
    const buffer = await this.accountingReportsService.exportProfitAndLossToExcel(
      data,
      'profit-and-loss',
    );
  ...
  @ApiOperation({
    ...
    description: 'Shows all transactions for an account over a period with running balance. Most detailed transaction report.',
  })
  @ApiQuery({
    name: 'accountId',
    required: true,
  ...
  async getGeneralLedger(
    ...
    if (!accountId || !startDate || !endDate) {
      throw new BadRequestException('accountId, startDate, and endDate are required');
    }
  ...
  @ApiOperation({
    ...
    description: 'Downloads General Ledger report for a specific account as an Excel file',
  })
  @ApiQuery({
    name: 'accountId',
    required: true,
  ...
  async exportGeneralLedger(
    ...
    const buffer = await this.accountingReportsService.exportGeneralLedgerToExcel(
      data,
      'general-ledger',
    );
  ...
  @ApiOperation({
    ...
    description: 'Enhanced version of General Ledger with status, reference links, and drill-down capability. Includes ALL entry statuses (DRAFT, POSTED, REVERSED) by default.',
  })
  @ApiQuery({
    name: 'accountId',
    required: true,
  ...
  async getAccountActivity(
    ...
    if (status) {
      if (!Object.values(JournalEntryStatus).includes(status as JournalEntryStatus)) {
        throw new BadRequestException(`Invalid status. Must be one of: ${Object.values(JournalEntryStatus).join(', ')}`);
      }
  ...
  @ApiOperation({
    ...
    description: 'Downloads Account Activity report for a specific account as an Excel file with color-coded status',
  })
  @ApiQuery({
    name: 'accountId',
    required: true,
  ...
  async exportAccountActivity(
    ...
    const buffer = await this.accountingReportsService.exportAccountActivityToExcel(
      data,
      'account-activity',
    );
