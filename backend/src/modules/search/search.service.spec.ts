...
describe("SearchService", () => {
  ...
  let purchaseOrderService: jest.Mocked<
    Pick<PurchaseOrderService, "searchGlobal">
  >;
  ...
  >;
  let searchAnalyticsService: jest.Mocked<
    Pick<SearchAnalyticsService, "logQuery">
  >;
  ...
  it("fans out to all ten sources in parallel", async () => {
    ...
    expect(salesOrderService.searchGlobal).toHaveBeenCalledWith(
      "abc",
      mockUser,
    );
    expect(purchaseOrderService.searchGlobal).toHaveBeenCalledWith(
      "abc",
      mockUser,
    );
  ...
  describe("searchPages scoring", () => {
    ...
    it("scores a page starts-with match as SCORE_PAGE_STARTSWITH + BOOST_PAGE (75)", async () => {
      ...
      const paymentsPage = result.results.find(
        (r) => r.route === "/sales/payments",
      );
    ...
    it("scores a keyword-only match as SCORE_PAGE_KEYWORD + BOOST_PAGE (50)", async () => {
      ...
      const customersPage = result.results.find(
        (r) => r.route === "/sales/customers",
      );
