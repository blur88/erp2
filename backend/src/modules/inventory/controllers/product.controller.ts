...
export class ProductController {
  ...
  @ApiQuery({
    name: "outOfStock",
    required: false,
    description: "Filter out of stock items only",
  })
  ...
  @ApiOperation({
    summary:
      "Check if product name or barcode already exists (including soft-deleted)",
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiQuery({
    name: "excludeId",
    required: false,
    description: "Product ID to exclude from check (for updates)",
  })
  ...
  @ApiOperation({
    summary: "Get dynamic pricing recommendations for a product",
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    summary: "Get order history for a product (sales and purchase orders)",
  })
  @ApiResponse({
    status: 200,
  ...
  async bulkUpdatePrices(
    ...
    return {
      message: `Successfully updated prices for ${bulkUpdateDto.products.length} products`,
    };
  }
  ...
  async reserveStock(
    ...
    const success = await this.productService.reserveStock(id, body.quantity);
  ...
  async releaseReservedStock(
    ...
    await this.productService.releaseReservedStock(id, body.quantity);
  ...
  @ApiResponse({
    status: 400,
    description: "Invalid product IDs or products are not deleted",
  })
  @ApiBody({
    schema: {
      type: "object",
  ...
  @ApiResponse({
    status: 400,
    description: "Invalid product IDs or products have active references",
  })
  @ApiBody({
    schema: {
      type: "object",
  ...
  async permanentDelete(
    ...
    await this.productService.permanentDelete(
      id,
      currentUserId,
      currentUsername,
    );
  ...
  @Delete(":id")
  @ApiOperation({
    summary: "Delete a product (soft delete - sets status to DISCONTINUED)",
  })
  @ApiResponse({
    status: 204,
