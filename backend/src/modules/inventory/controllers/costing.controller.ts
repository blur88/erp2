...
export class CostingController {
  ...
  @ApiOperation({
    ...
    description: 'Returns the currently configured costing method (AVERAGE, FIFO, LIFO, STANDARD)',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description: 'Recalculates base costs for all products using the current costing method. ' +
  ...
  async recalculateAllCosts() {
    ...
    const result = await this.costingRecalculationService.recalculateAllProductCosts();
    this.logger.log(
  ...
  @ApiOperation({
    ...
    description: 'Recalculates base cost for a specific product using the current costing method',
  })
  @ApiParam({
    name: 'id',
  ...
  async recalculateProductCost(@Param('id') productId: string) {
    ...
    return await this.costingRecalculationService.recalculateProductCost(productId);
  }
}
