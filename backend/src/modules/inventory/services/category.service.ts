...
> {
  ...
  protected applyQueryBuilder(qb: any, query: QueryCategoriesDto): any {
    ...
    if (query.parentId !== undefined) {
      if (query.parentId === null) {
        ...
        qb = qb.andWhere("category.parentId = :parentId", {
    ...
    const sortField = ["name", "createdAt"].includes(query.sortBy ?? "")
      ? query.sortBy!
      : "name";
    if (sortField === "name") {
  ...
  protected applySearch(qb: any, search: string, _alias: string): any {
    return qb.andWhere("category.name ILIKE :search", {
      search: `%${search}%`,
    });
  }
  ...
  private buildCategoryListQuery(
  ...
  async create(
    ...
    if (createCategoryDto.parentId) {
      ...
      if (!parent) {
        throw new NotFoundException(
          `Parent category with ID '${createCategoryDto.parentId}' not found or inactive`,
        );
      }
    }
    ...
    const path = parent
      ? `${parent.path || parent.name}.${createCategoryDto.name}`
      : createCategoryDto.name;
  ...
  async findDeleted(
    ...
    const deletedQuery = {
      ...query,
      sortBy: this.allowedSortFields.includes(query.sortBy ?? "")
        ? query.sortBy
        : "deletedAt",
      sortOrder: query.sortOrder ?? "DESC",
    } as QueryCategoriesDto;
    const queryBuilder = this.buildCategoryListQuery(deletedQuery, {
      includeDeleted: true,
    });
    ...
    const data = await Promise.all(
      categories.map(
        async (category) => await this.toResponseDto(category, false, false),
      ),
    );
    ...
    try {
      ...
      const maxDepth =
        allCategories.length > 0
          ? Math.max(...allCategories.map((c) => c.level), 0)
          : 0;
      const rootCategoriesCount = allCategories.filter(
        (c) => c.level === 0,
      ).length;
  ...
  async getAncestors(id: string): Promise<CategoryAncestorsDto> {
    ...
    return {
      ...
      ancestors: await Promise.all(
        ancestors.map((ancestor) => this.toResponseDto(ancestor)),
      ),
  ...
  async update(
    ...
    const parentChanged =
      updateCategoryDto.hasOwnProperty("parentId") &&
    ...
    this.logger.log(
      `Name changed: ${nameChanged}, Parent changed: ${parentChanged}`,
    );
    this.logger.log(
      `Old parent: ${category.parentId}, New parent: ${updateCategoryDto.parentId}`,
    );
    ...
    if (parentChanged && updateCategoryDto.parentId) {
      ...
      if (!newParent) {
        throw new NotFoundException(
          `New parent category with ID '${updateCategoryDto.parentId}' not found`,
        );
      }
    ...
    if (nameChanged || parentChanged) {
      this.logger.log(
        `Updating paths and levels for category ${updatedCategory.name} and its descendants`,
      );
      ...
      if (updatedCategory.parentId) {
        ...
        if (parent) {
          ...
          updatedCategory.path = parent.path
            ? `${parent.path}.${updatedCategory.name}`
            : `${parent.name}.${updatedCategory.name}`;
        }
      } else {
      ...
      this.logger.log(
        `Updated category level: ${updatedCategory.level}, path: "${updatedCategory.path}"`,
      );
      ...
      this.logger.log(
        `Finished updating descendant paths for category ${updatedCategory.name}`,
      );
    }
  ...
  async moveCategory(
    ...
    this.logger.log(
      `Moving category ${id} to parent ${moveCategoryDto.newParentId}`,
    );
    ...
    if (moveCategoryDto.newParentId) {
      ...
      if (!newParent) {
        throw new NotFoundException(
          `New parent category with ID '${moveCategoryDto.newParentId}' not found`,
        );
      }
  ...
  async remove(
    ...
    this.logger.log(
      `Found category: ${category.name} with ${activeProductCount} active products and ${category.children?.length || 0} children`,
    );
    ...
    if (activeProductCount > 0) {
      ...
      if (options.moveToUncategorized) {
        ...
        this.logger.log(
          `Category deleted successfully with ${movedCount} products moved to Uncategorized`,
        );
        return {
          moved: movedCount,
  ...
  async permanentDelete(
    ...
    if (activeDependencies.length > 0) {
      throw new BadRequestException(
        ValidationUtil.createDependencyErrorMessage(
          "category",
          activeDependencies,
        ),
      );
  ...
  async bulkRestore(
    ...
    for (const categoryId of categoryIds) {
      try {
        ...
        this.logger.error(
          `Failed to restore category ${categoryId}: ${error.message}`,
        );
        BulkOperationUtil.addFailure(
          failedItems,
          categoryId,
  ...
  async bulkPermanentDelete(
    ...
    this.logger.log(
      `Bulk permanently deleting ${categoryIds.length} categories`,
    );
    ...
    for (const categoryId of categoryIds) {
      try {
        ...
        this.logger.error(
          `Failed to permanently delete category ${categoryId}: ${error.message}`,
        );
        BulkOperationUtil.addFailure(
          failedItems,
          categoryId,
   ...
   */
  async bulkUpdate(
    bulkUpdateDto: BulkUpdateCategoriesDto,
    userId?: string,
  ): Promise<void> {
    this.logger.log(
      `Bulk updating ${bulkUpdateDto.categories.length} categories`,
    );
    ...
    const updates = bulkUpdateDto.categories.map(async (categoryUpdate) => {
      ...
      ["name", "parentId"].forEach((field) => {
        if (
          categoryUpdate[field] !== undefined &&
          categoryUpdate[field] !== category[field]
        ) {
  ...
  async getCategoryStats(id: string): Promise<CategoryStatsDto> {
    ...
    const [
      ...
      directProductsQuery
        .clone()
        .andWhere("product.stockQuantity <= product.reorderLevel")
        .getCount(),
      directProductsQuery
        .clone()
  ...
  async checkDuplicate(params: {
    ...
    this.logger.log(
      `Checking duplicate for category name: "${params.name}", parentId: "${params.parentId}"`,
    );
    ...
    if (params.name && params.name.trim()) {
      ...
      if (params.parentId) {
        nameQuery.andWhere("category.parentId = :parentId", {
      ...
      if (params.excludeId) {
        nameQuery.andWhere("category.id != :excludeId", {
   ...
   */
  private async updateDescendantLevelsAndPaths(
    category: Category,
  ): Promise<void> {
    const descendants = await this.loadAllDescendants(category);
   ...
   */
  private async moveProductsToUncategorized(
    categoryId: string,
    userId?: string,
  ): Promise<number> {
    this.logger.log(
      `Moving products from category ${categoryId} to Uncategorized`,
    );
    ...
    if (!uncategorizedCategory) {
      ...
      uncategorizedCategory = await this.categoryRepository.save(
        uncategorizedCategory,
      );
      this.logger.log(
        `Created Uncategorized category with ID: ${uncategorizedCategory.id}`,
      );
    } else if (uncategorizedCategory.deletedAt) {
      // Restore soft-deleted Uncategorized category
      this.logger.log(
        `Restoring soft-deleted Uncategorized category with ID: ${uncategorizedCategory.id}`,
      );
      ...
      this.logger.log(
        `Restored Uncategorized category with ID: ${uncategorizedCategory.id}`,
      );
    } else {
      this.logger.log(
        `Found existing active Uncategorized category with ID: ${uncategorizedCategory.id}`,
      );
    }
    ...
    this.logger.log(
      `Updating ${totalProductCount} products from category ${categoryId} to ${uncategorizedCategory.id}`,
    );
    ...
    this.logger.log(
      `Products moved successfully. Remaining products in old category: ${remainingCount[0]?.count || 0}`,
    );
