...
export class BaseCostCalculatorService {
   ...
   */
  async calculateBaseCostFromCurrentStock(
    productId: string,
    manager?: EntityManager,
  ): Promise<number> {
    const productRepo = repoFor(manager, Product, this.productRepository);
    const costHistoryRepo = repoFor(
      manager,
      PurchaseCostHistory,
      this.costHistoryRepository,
    );
    ...
    this.logger.debug(
      `Calculating base cost for product ${productId} using ${costingMethod} method`,
    );
   ...
   */
  async updateProductBaseCost(
    productId: string,
    manager?: EntityManager,
  ): Promise<number> {
    const productRepo = repoFor(manager, Product, this.productRepository);
    const newBaseCost = await this.calculateBaseCostFromCurrentStock(
      productId,
      manager,
    );
    ...
    this.logger.log(
      `Updated product ${productId} base cost to RM ${newBaseCost.toFixed(4)}`,
    );
  ...
  async reduceStock(
    ...
    this.logger.log(
      `Reducing stock for product ${productId}: ${quantitySold} units`,
    );
    ...
    if (batches.length === 0) {
      this.logger.warn(
        `No cost history batches found for product ${productId}, cannot reduce stock`,
      );
      return;
    }
  ...
  async removeStock(productId: string, grnId: string): Promise<void> {
    this.logger.log(
      `Removing stock for product ${productId} from GRN ${grnId}`,
    );
    ...
    if (batches.length === 0) {
      this.logger.warn(
        `No cost history batches found for product ${productId} and GRN ${grnId}`,
      );
      return;
    }
    ...
    this.logger.log(
      `Deleted ${batches.length} cost history batch(es) for GRN ${grnId}`,
    );
   ...
   */
  async getCurrentStockBatches(
    productId: string,
  ): Promise<PurchaseCostHistory[]> {
    return await this.costHistoryRepository.find({
      where: {
        productId,
        remainingQuantity: MoreThan(0),
  ...
  async restoreStock(
    ...
    this.logger.log(
      `Restoring stock for product ${productId}: ${quantityToRestore} units`,
    );
    ...
    if (batches.length === 0) {
      this.logger.warn(
        `No cost history batches found for product ${productId}, cannot restore stock`,
      );
      return;
    }
