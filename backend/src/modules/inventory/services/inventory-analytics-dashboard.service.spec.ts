...
function makeQb(rawResult: any = {}, manyResult: any[] = []) {
  const qb: any = {
    ...
    getRawAndEntities: jest
      .fn()
      .mockResolvedValue({ entities: manyResult, raw: [] }),
  };
  return qb;
}
...
describe("InventoryAnalyticsService.getInventoryDashboardAnalytics", () => {
  ...
  beforeEach(async () => {
    ...
    settingsService = {
      getRegionalSettings: jest
        .fn()
        .mockResolvedValue({ lowStockThreshold: 10, timezone: "UTC" }),
    };
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(PriceListItem), useValue: mockRepo() },
        {
          ...
          useValue: purchaseOrderItemRepo,
        },
  ...
  it("returns comparison block with periodStart before current periodStart for previous_period", async () => {
    ...
    expect(result.comparison!.periodStart < result.current.periodStart).toBe(
      true,
    );
    expect(result.comparison!.periodEnd < result.current.periodStart).toBe(
      true,
    );
  });
  ...
  describe("dashboard filters", () => {
    function setupDefaultMocksWithFilters() {
      purchaseOrderItemRepo.createQueryBuilder = jest
        .fn()
    ...
    it("passes stockStatus low_stock andWhere to productRepo with 0 < stockQuantity <= 10", async () => {
      ...
      await service.getInventoryDashboardAnalytics({
