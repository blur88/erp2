...
describe("BackupService - settings backup", () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getRepositoryToken(BackupRetentionSettings),
        ...
        {
          provide: getRepositoryToken(RegionalSettings),
          useFactory: mockRepository,
        },
        {
          provide: getRepositoryToken(DocumentNumberSetting),
          useFactory: mockRepository,
        },
        {
          provide: getRepositoryToken(PrintSettings),
          useFactory: mockRepository,
        },
      ],
    }).compile();
    ...
    documentNumberSettingRepo = module.get(
      getRepositoryToken(DocumentNumberSetting),
    );
  ...
  describe("processUploadedBackup", () => {
    ...
    it("stores uploaded backups under a generated archive filename and preserves the original name in metadata", async () => {
      ...
      jest
        ...
        .mockResolvedValue(JSON.stringify({ description: "Uploaded backup" }));
      jest.spyOn(fsPromises, "stat").mockResolvedValue({ size: 42 });
      jest
        .spyOn(fsPromises, "readdir")
        .mockResolvedValue(["erp_db_20260430_120000.sql.gz"]);
      ...
      jest
        ...
        .mockResolvedValue("checksum-123");
      ...
      expect(result.filename).toBe(
        "uploaded_backup_1770000000000_uuid-123.tar.gz",
      );
  ...
  describe("command execution", () => {
    it("uses spawn argument arrays for PostgreSQL backup creation", async () => {
      ...
      const result = await (service as any).backupPostgreSQL(
        "/tmp/backup dir",
        "20260430_120000",
      );
    ...
    it("uses spawn argument arrays for tar extraction", async () => {
      ...
      await (service as any).extractArchive(
      ...
      expect(mockSpawn).toHaveBeenCalledWith(
        "tar",
        ["-xzf", "/tmp/backup;rm.tar.gz", "-C", "/tmp/restore dir"],
        {},
      );
    });
    ...
    it("uses spawn argument arrays for PostgreSQL restore", async () => {
      ...
      expect(mockSpawn).toHaveBeenNthCalledWith(
        ...
        [
          ...
          "erp_db",
          "-f",
          "/tmp/restore dir/erp_db_20260430_120000.sql",
        ],
        expect.objectContaining({
          env: expect.objectContaining({ PGPASSWORD: "" }),
        }),
    ...
    it("rejects when a spawned command exits with a non-zero code", async () => {
      ...
      await expect(
        (service as any).backupPostgreSQL("/tmp", "20260430_120000"),
    ...
    it("rejects when a spawned command is killed by a signal", async () => {
      ...
      await expect(
        (service as any).extractArchive("/tmp/backup.tar.gz", "/tmp/restore"),
  ...
  describe("getCompanySettings", () => {
    it("returns company settings data from repository", async () => {
      ...
      expect(companySettingsRepo.findOne).toHaveBeenCalledWith({
        where: { isActive: true },
      });
  ...
  describe("getPrintSettings", () => {
    it("returns print settings data from repository", async () => {
      ...
      expect(result.salesOrderTemplate).toEqual({
        title: "Sales Order",
        showLogo: true,
      });
  ...
  describe("backupSettings", () => {
    it("writes settings JSON file with all 4 settings types", async () => {
      ...
      jest
        ...
        .mockImplementation(mockFs.writeFile);
      ...
      expect(data.printSettings.salesOrderTemplate).toEqual({
        title: "Sales Order",
      });
  ...
  describe("restoreSettings", () => {
    const mockSettingsJson = {
      companySettings: {
        name: "Restored Corp",
        address: "2 Restore St",
        city: "Petaling Jaya",
        state: "Selangor",
        postalCode: "47500",
        country: "Malaysia",
        phone: "03-99999999",
        email: "restore@corp.com",
        website: "",
        miscInfo: "",
      },
      regionalSettings: {
    ...
    beforeEach(() => {
      jest
        .spyOn(require("fs/promises"), "readdir")
        .mockResolvedValue(["settings_20260223_120000.json"]);
      jest
        .spyOn(require("fs/promises"), "readFile")
        .mockResolvedValue(JSON.stringify(mockSettingsJson));
    });
    ...
    it("upserts existing company settings (update path)", async () => {
      ...
      companySettingsRepo.save.mockResolvedValue({
        ...existing,
        ...mockSettingsJson.companySettings,
      });
    ...
    it("does not restore logoUrl for company settings", async () => {
      const jsonWithLogo = {
        ...
        companySettings: {
          ...mockSettingsJson.companySettings,
          logoUrl: "/uploads/logos/old.png",
        },
      };
      jest
        .spyOn(require("fs/promises"), "readFile")
        .mockResolvedValue(JSON.stringify(jsonWithLogo));
    ...
    it("restores regional settings from old backup using priceCostingSettings key", async () => {
      ...
      jest
        .spyOn(require("fs/promises"), "readFile")
        .mockResolvedValue(JSON.stringify(oldBackupJson));
      ...
      expect(regionalSettingsRepo.findOne).toHaveBeenCalledWith({
        where: { isActive: true },
      });
    ...
    it("logs warning and skips if no settings file found", async () => {
      jest
        .spyOn(require("fs/promises"), "readdir")
        .mockResolvedValue(["other_file.json"]);
...
describe("BackupService - createArchive", () => {
  ...
  afterEach(async () => {
    ...
    await require("fs/promises").rm(parentDir, {
      recursive: true,
      force: true,
    });
  });
  ...
  it("resolves to the output path and produces a non-empty .tar.gz file", async () => {
    const result = await (service as any).createArchive(
      tempSourceDir,
      tempOutputPath,
    );
