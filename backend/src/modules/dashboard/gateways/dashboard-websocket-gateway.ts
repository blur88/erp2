...
export class DashboardWebSocketGateway {
  ...
  async handleDashboardDataRequest(
    ...
    try {
      const dashboardData = await this.dashboardService.getDashboardData(request);
  ...
  async broadcastAlerts() {
    try {
      const alerts: DashboardAlert[] = await this.dashboardService.generateAlerts();
  ...
  async startPeriodicRefresh(intervalMs: number = 300000) { // Default 5 minutes
    setInterval(async () => {
