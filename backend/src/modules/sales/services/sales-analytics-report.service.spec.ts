...
describe("SalesAnalyticsReportService", () => {
  ...
  it("builds product summary metrics from sales and purchase items", async () => {
    ...
    const purchaseQueryBuilder = {
      ...
      where: jest.fn().mockReturnThis(),
      andWhere: jest.fn().mockReturnThis(),
      getMany: jest.fn().mockResolvedValue([{ quantity: 5, unitCost: 18 }]),
    };

    salesOrderItemRepository.createQueryBuilder.mockReturnValue(
      salesQueryBuilder as any,
    );
    purchaseOrderItemRepository.createQueryBuilder.mockReturnValue(
      purchaseQueryBuilder as any,
    );
