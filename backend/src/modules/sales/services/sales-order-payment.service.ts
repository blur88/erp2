...
export class SalesOrderPaymentService {
  ...
  computePaymentStatus(
    ...
    if (Math.abs(netPaid - total) < AMOUNT_TOLERANCE)
  ...
  async recordPayment(
    ...
    if (!method)
      throw new BadRequestException(
        `Payment method ${dto.paymentMethodId} not found or inactive`,
      );

    const { saved, orderNumber } = await this.dataSource.transaction(
      async (manager: EntityManager) => {
        ...
        if (order.status !== SalesOrderStatus.DRAFT) {
          throw new ConflictException(
            "Payments can only be recorded on DRAFT orders",
          );
        }
        ...
        const savedRecord = await manager
    ...
    await this.auditLogService.log(
      "CREATE",
      "SalesOrderPayment",
      `Recorded payment for ${orderNumber}`,
      {
        entityId: saved.id,
  ...
  async recordRefund(
    ...
    const { saved, resultingStatus, orderNumber } =
      await this.dataSource.transaction(async (manager: EntityManager) => {
        ...
        if (order.status === SalesOrderStatus.CANCELLED) {
          throw new ConflictException(
            "Cannot record a refund on a cancelled order",
          );
        }
        ...
        if (dto.amount > netPaid + AMOUNT_TOLERANCE) {
          throw new BadRequestException(
            `Refund amount (${dto.amount}) exceeds net paid (${netPaid.toFixed(4)})`,
          );
        }
        ...
        const savedRecord = await manager
          .getRepository(SalesOrderPayment)
          .save(record);
        const resultingStatus = await this.updatePaymentStatusInTx(
          order,
          manager,
        );
        return {
    ...
    await this.auditLogService.log(
      "CREATE",
      "SalesOrderPayment",
      `Recorded refund for ${orderNumber} (status: ${resultingStatus})`,
      {
        entityId: saved.id,
        userId: userId || "system",
        username,
  ...
  async recordPayments(
    ...
    const { results, orderNumber } = await this.dataSource.transaction(
      async (manager: EntityManager) => {
        const order = await lockRowForUpdate(manager, SalesOrder, orderId, {
          notFoundMessage: "Sales order not found",
        });
        ...
        for (const dto of dtos) {
          ...
          saved.push(
            await manager.getRepository(SalesOrderPayment).save(record),
          );
        }
        await this.updatePaymentStatusInTx(order, manager);
  ...
  async recordRefunds(
    ...
    const { results, resultingStatus, orderNumber } =
      await this.dataSource.transaction(async (manager: EntityManager) => {
        const order = await lockRowForUpdate(manager, SalesOrder, orderId, {
          notFoundMessage: "Sales order not found",
        });
        ...
        const existing = await manager
          ...
          .find({ where: { salesOrderId: orderId } });
        const netPaid = existing.reduce((sum, r) => sum + Number(r.amount), 0);
        const totalRefundAmount = dtos.reduce(
          (sum, dto) => sum + dto.amount,
          0,
        );
        if (totalRefundAmount > netPaid + AMOUNT_TOLERANCE) {
          throw new BadRequestException(
            `Total refund amount (${totalRefundAmount}) exceeds net paid (${netPaid.toFixed(4)})`,
          );
        }
  ...
  private async updatePaymentStatusInTx(
    order: SalesOrder,
    manager: EntityManager,
  ): Promise<SalesOrderPaymentStatus> {
    ...
    const paidInFull =
      newStatus === SalesOrderPaymentStatus.PAID ||
      newStatus === SalesOrderPaymentStatus.OVERPAID;
