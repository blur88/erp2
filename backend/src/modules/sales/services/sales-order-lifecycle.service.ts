...
export class SalesOrderLifecycleService {
  ...
  static assertStatusEditable(status: SalesOrderStatus): void {
    const editable =
      status === SalesOrderStatus.DRAFT || status === SalesOrderStatus.READY;
    if (!editable) {
      throw new BadRequestException(
        `Cannot edit sales order in ${status} status. ${
          status === SalesOrderStatus.FULFILLED
            ? "Unfulfill the order first."
  ...
  async cancel(
      ...
      async (manager: EntityManager) => {
        ...
        if (order.status === SalesOrderStatus.FULFILLED) {
          throw new ConflictException(
            "Cannot cancel a fulfilled order. Unfulfill it first.",
          );
        }
        ...
        await manager
          ...
          .update(id, { status: SalesOrderStatus.CANCELLED });
    ...
    await this.auditLogService.log(
      "UPDATE",
      "SalesOrder",
      `Cancelled sales order: ${saved.orderNumber}`,
      {
        entityId: id,
  ...
  async uncancel(
    ...
    const saved = await this.dataSource.transaction(
      async (manager: EntityManager) => {
        ...
        await manager
          .getRepository(SalesOrder)
          .update(id, { status: SalesOrderStatus.DRAFT });
    ...
    await this.auditLogService.log(
      "UPDATE",
      "SalesOrder",
      `Uncancelled sales order: ${saved.orderNumber}`,
      {
        entityId: id,
        userId: userId || "system",
        username,
