...
describe("SalesOrderQueryService", () => {
  ...
  describe("findAll", () => {
    ...
    it("applies persisted status=READY directly", async () => {
      ...
      const statusClause = andWhereCalls.find(
        (c) => c.clause === "order.status = :status",
      );
      const paymentClause = andWhereCalls.find(
        (c) => c.clause === "order.paymentStatus = :ps",
      );
    ...
    it("applies a paymentStatus param independently of READY", async () => {
      ...
      const paymentClauses = andWhereCalls.filter((c) =>
        c.clause.startsWith("order.paymentStatus"),
      );
