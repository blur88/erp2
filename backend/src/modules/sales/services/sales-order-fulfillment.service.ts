      ...
      async (manager: EntityManager) => {
        ...
        if (order.status !== SalesOrderStatus.READY) {
          throw new ConflictException(
            "Cannot fulfill order - order must be Ready (paid in full)",
          );
        }
        ...
        this.logger.log(
          `Posted accounting entry for sales order ${order.orderNumber}`,
        );
  ...
  async unfulfillOrder(
    ...
    const saved = await this.dataSource.transaction(
      async (manager: EntityManager) => {
        ...
        for (const item of order.items) {
          if (item.product) {
            await this.baseCostCalculator.restoreStock(
              item.productId,
              item.quantity,
              manager,
            );
        ...
        this.logger.log(
          `Deleted ${result.deletedCount} stock movements for ${order.orderNumber}`,
        );
        ...
        this.logger.log(
          `Reversed accounting entry for sales order ${order.orderNumber}`,
        );
