...
describe("SalesOrderPaymentService", () => {
          ...
          return {
            ...
            save: jest
              ...
              .mockImplementation((data) =>
                Promise.resolve({ id: "payment-new", ...data }),
              ),
  ...
  describe("computePaymentStatus", () => {
    it("returns UNPAID when no payments", () => {
      expect(service.computePaymentStatus([], 1000)).toBe(
    ...
    it("returns UNPAID when net paid is negative (over-refunded edge case)", () => {
      ...
      expect(service.computePaymentStatus(records, 1000)).toBe(
        SalesOrderPaymentStatus.UNPAID,
      );
    });
    ...
    it("returns PARTIAL when net paid < total", () => {
      ...
      expect(service.computePaymentStatus(records, 1000)).toBe(
        SalesOrderPaymentStatus.PARTIAL,
      );
    });
    ...
    it("returns OVERPAID when net paid > total", () => {
      ...
      expect(service.computePaymentStatus(records, 1000)).toBe(
        SalesOrderPaymentStatus.OVERPAID,
      );
    });
  ...
  describe("reconcileOrderState", () => {
    ...
    it("demotes READY -> DRAFT + PARTIAL when total rises above amount paid", async () => {
      ...
      expect(captured.update.paymentStatus).toBe(
        SalesOrderPaymentStatus.PARTIAL,
      );
  ...
  describe("recordPayment", () => {
        ...
        getRepository: jest.fn().mockImplementation((entity) => {
          if (entity === SalesOrderPayment) {
            return {
              ...
              save: jest
                .fn()
                .mockImplementation((d) => Promise.resolve({ id: "p1", ...d })),
    ...
    it("throws ConflictException in-lock when the order is no longer DRAFT", async () => {
      const findOne = jest
        .fn()
        .mockResolvedValue(mockOrder({ status: SalesOrderStatus.READY }));
      const manager = {
        getRepository: jest
          .fn()
        ...
        getRepository: jest.fn().mockImplementation((entity) => {
          if (entity === SalesOrderPayment) {
            return {
              ...
              find: jest
                .fn()
                .mockResolvedValue([{ amount: 400 }] as SalesOrderPayment[]),
            };
          }
          if (entity === SalesOrder)
            return {
        ...
        getRepository: jest.fn().mockImplementation((entity) => {
          if (entity === SalesOrderPayment) {
            return {
              ...
              find: jest
                .fn()
                .mockResolvedValue([{ amount: 1200 }] as SalesOrderPayment[]),
            };
          }
          if (entity === SalesOrder)
            return {
  ...
  describe("READY status reconciliation", () => {
    it("flips DRAFT -> READY when a payment pays the order in full", async () => {
      ...
      const update = jest.fn().mockResolvedValue(undefined);
      const manager = buildMockManager(
        [{ amount: 100 }] as SalesOrderPayment[],
        update,
        order,
      );
      (dataSource.transaction as jest.Mock).mockImplementation(
    ...
    it("flips DRAFT -> READY on overpayment", async () => {
      ...
      const update = jest.fn().mockResolvedValue(undefined);
      const manager = buildMockManager(
        [{ amount: 120 }] as SalesOrderPayment[],
        update,
        order,
      );
      (dataSource.transaction as jest.Mock).mockImplementation(
    ...
    it("leaves status DRAFT when a partial payment does not reach full", async () => {
      ...
      const update = jest.fn().mockResolvedValue(undefined);
      const manager = buildMockManager(
        [{ amount: 30 }] as SalesOrderPayment[],
        update,
        order,
      );
      (dataSource.transaction as jest.Mock).mockImplementation(
  ...
  describe("recordPayments", () => {
    ...
    it("throws if any dto has invalid paymentMethodId", async () => {
      ...
      await expect(service.recordPayments("order-1", dtos)).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe("listPayments", () => {
    it("throws NotFoundException when order not found", async () => {
      ...
      await expect(service.listPayments("order-1")).rejects.toThrow(
        NotFoundException,
      );
    });
