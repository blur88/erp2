...
export class SalesOrderController {
  ...
  @ApiQuery({
    name: "status",
    required: false,
    enum: ["DRAFT", "FULFILLED", "CANCELLED"],
  })
  @ApiQuery({
    name: "paymentStatus",
    required: false,
  ...
  @Put(":id")
  @ApiOperation({
    summary: "Update sales order (DRAFT or READY; FULFILLED/CANCELLED locked)",
  })
  @ApiParam({ name: "id", type: "string" })
  ...
  async fulfillOrder(
    ...
    const data = await this.salesOrderService.fulfillOrder(
      id,
      userId,
      username,
    );
  ...
  async unfulfillOrder(
    ...
    const data = await this.salesOrderService.unfulfillOrder(
      id,
      userId,
      username,
    );
  ...
  async recordPayment(
    ...
    const data = await this.salesOrderService.recordPayment(
      id,
      dto,
      userId,
      username,
  ...
  async recordPaymentsBatch(
    ...
    const data = await this.salesOrderService.recordPayments(
      id,
      dto.payments,
      userId,
      username,
  ...
  @Post(":id/refunds")
  @ApiOperation({
    summary: "Record one or more refunds (creates negative payment records)",
  })
  async recordRefunds(
    @Param("id", ParseUUIDPipe) id: string,
    ...
    const data = await this.salesOrderService.recordRefunds(
      id,
      dto.refunds,
      userId,
      username,
