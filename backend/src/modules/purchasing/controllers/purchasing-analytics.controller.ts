...
export class PurchasingAnalyticsController {
  ...
  @Get("dashboard")
  @ApiOperation({
    summary: "Get purchasing analytics for the overview dashboard",
  })
  ...
  async exportPurchaseOrderSummary(
    ...
    const { data } =
      await this.purchasingAnalyticsService.getPurchaseOrderSummary({
  ...
  async exportPurchaseOrderStatus(
    ...
    const { data } =
      await this.purchasingAnalyticsService.getPurchaseOrderDetails({
        dateFrom: toDate(dateFrom),
        dateTo: toDate(dateTo),
        supplierId,
        categoryId,
  ...
  async exportVendorPaymentDetails(
    ...
    const { data } =
      await this.purchasingAnalyticsService.getVendorPaymentDetails({
        dateFrom: toDate(dateFrom),
        dateTo: toDate(dateTo),
        supplierId,
        status,
