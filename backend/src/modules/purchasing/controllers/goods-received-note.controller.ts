...
export class GoodsReceivedNoteController {
  ...
  @ApiOperation({
    ...
    description: 'Create a new goods received note for a purchase order with receipt details and quality inspection tracking.'
  })
  @ApiResponse({
    status: 201,
  ...
  @ApiOperation({
    ...
    description: 'Retrieve goods received notes with advanced filtering, searching, and pagination capabilities.'
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiQuery({ name: 'search', required: false, type: String, description: 'Search by GRN number, PO number, or supplier name' })
  @ApiQuery({ name: 'status', required: false, description: 'Filter by status' })
  @ApiQuery({ name: 'type', required: false, description: 'Filter by type' })
  @ApiQuery({ name: 'supplierId', required: false, type: String, description: 'Filter by supplier' })
  @ApiQuery({ name: 'purchaseOrderId', required: false, type: String, description: 'Filter by purchase order' })
  @ApiQuery({ name: 'sortBy', required: false, type: String, description: 'Sort by field (default: receivedDate)' })
  @ApiQuery({ name: 'sortOrder', required: false, enum: ['ASC', 'DESC'], description: 'Sort order (default: DESC)' })
  async findAll(@Query() query: GoodsReceivedNoteQueryDto): Promise<GoodsReceivedNoteListResponseDto> {
  ...
  @ApiOperation({
    ...
    description: 'Retrieve all soft-deleted goods received notes with pagination.'
  })
  @ApiResponse({
    status: 200,
  ...
  async findDeleted(@Query() query: GoodsReceivedNoteQueryDto): Promise<GoodsReceivedNoteListResponseDto> {
  ...
  @ApiOperation({
    ...
    description: 'Retrieve detailed information about a specific goods received note including all line items and quality inspection results.'
  })
  @ApiResponse({
    status: 200,
  ...
  async findOne(@Param('id', ParseUUIDPipe) id: string): Promise<GoodsReceivedNoteResponseDto> {
  ...
  @ApiOperation({
    ...
    description: 'Update GRN information including delivery details, inspection status, and notes.'
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description: 'Restore multiple soft-deleted goods received notes to active status.'
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description: 'Permanently delete multiple soft-deleted goods received notes from the database. This action cannot be undone.'
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description: 'Permanently delete a soft-deleted goods received note from the database. This action cannot be undone.'
  })
  ...
  @ApiOperation({
    ...
    description: 'Soft delete (deactivate) goods received note. Can be restored later if needed.'
  })
