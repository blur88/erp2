...
export class UsersController {
  ...
  @ApiOperation({
    ...
    description:
      "Retrieve users with filtering, searching, and pagination (Admin/Manager only)",
  })
  @ApiResponse({
    status: 200,
  ...
  async findAll(
    ...
    try {
      this.logger.log(
        `Retrieving users with query: ${JSON.stringify(queryDto)}`,
      );
  ...
  async adminUpdate(
    ...
    try {
      ...
      this.logger.error(
        `Admin user update failed: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  async remove(
    ...
    try {
      ...
      this.logger.error(
        `User deactivation failed: ${error.message}`,
        error.stack,
      );
      throw error;
    }
