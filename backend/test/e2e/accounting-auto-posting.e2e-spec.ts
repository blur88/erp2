...
import {
  JournalEntry,
  JournalEntryStatus,
} from "../../src/database/entities/journal-entry.entity";
...
describe("Accounting Auto-Posting Integration (E2E)", () => {
  ...
  async function setupFiscalPeriods() {
    ...
    const savedOpen = (await fiscalPeriodRepo.save(
      openPeriod,
    ...
    const savedClosed = (await fiscalPeriodRepo.save(
      closedPeriod,
    )) as unknown as FiscalPeriod;
  ...
  async function setupChartOfAccounts() {
    ...
    for (const data of accountsData) {
      ...
      const saved = (await chartOfAccountRepo.save(
        account,
  ...
  async function setupAccountMappings() {
    const mappings = [
      ...
      {
        type: MappingType.INVENTORY_ADJUSTMENT_GAIN,
        accountId: accounts["7100"].id,
      },
      {
        type: MappingType.INVENTORY_ADJUSTMENT_LOSS,
        accountId: accounts["6100"].id,
      },
    ];
  ...
  async function setupPaymentMethods() {
    ...
    testPaymentMethodCash = (await paymentMethodRepo.save(
      cashMethod,
    ...
    testPaymentMethodBank = (await paymentMethodRepo.save(
      bankMethod,
    )) as unknown as PaymentMethodEntity;
  }
  ...
  describe("Sales Order Fulfillment Auto-Posting", () => {
    it("should auto-post journal entry when sales order fulfilled", async () => {
      ...
      const savedOrder = (await salesOrderRepo.save(
        salesOrder,
      ...
      // Fulfill the order
      const fulfilledOrder = await salesOrderService.fulfillOrder(
        savedOrder.id,
      );
      ...
      expect(
        journalEntries.every((e) => e.status === JournalEntryStatus.POSTED),
      ).toBe(true);
      ...
      const cogsLine = cogsEntry.lines.find(
        (l) => l.accountId === accounts["5000"].id && Number(l.debitAmount) > 0,
      );
      expect(cogsLine).toBeDefined();
      expect(Number(cogsLine.debitAmount)).toBe(500.0); // 10 * 50 baseCost

      const inventoryLine = cogsEntry.lines.find(
        (l) =>
          l.accountId === accounts["1300"].id && Number(l.creditAmount) > 0,
      );
      expect(inventoryLine).toBeDefined();
      expect(Number(inventoryLine.creditAmount)).toBe(500.0);
      ...
      const arLine = revenueEntry.lines.find(
        (l) => l.accountId === accounts["1200"].id && Number(l.debitAmount) > 0,
      );
      expect(arLine).toBeDefined();
      expect(Number(arLine.debitAmount)).toBe(1000.0);

      const revenueLine = revenueEntry.lines.find(
        (l) =>
          l.accountId === accounts["4000"].id && Number(l.creditAmount) > 0,
      );
      expect(revenueLine).toBeDefined();
      expect(Number(revenueLine.creditAmount)).toBe(1000.0);
    });
    ...
    it("should calculate COGS correctly from product baseCost", async () => {
      ...
      expect(Number(cogsLine.debitAmount)).toBe(500.0); // 10 qty * 50 baseCost
    });
    ...
    it("rolls back fulfillment when an accounting mapping is missing (atomic, #718/#719)", async () => {
      ...
      await expect(
        salesOrderService.fulfillOrder(savedOrder.id),
      ).rejects.toThrow(/Account mapping not configured/);

      // Order stays READY (status change rolled back).
      const reloaded = await salesOrderRepo.findOne({
  ...
  describe("Customer Payment Auto-Posting", () => {
    it("should auto-post journal entry when payment received", async () => {
      ...
      const cashLine = entry.lines.find(
        (l) => l.accountId === accounts["1100"].id && Number(l.debitAmount) > 0,
      );
      expect(cashLine).toBeDefined();
      expect(Number(cashLine.debitAmount)).toBe(500.0);

      // Verify AR credit
      const arLine = entry.lines.find(
        (l) =>
          l.accountId === accounts["1200"].id && Number(l.creditAmount) > 0,
      );
      expect(arLine).toBeDefined();
      expect(Number(arLine.creditAmount)).toBe(500.0);
    });
  ...
  describe("Goods Received Note Auto-Posting", () => {
    it("should auto-post journal entry when GRN created", async () => {
      ...
      const inventoryLine = entry.lines.find(
        (l) => l.accountId === accounts["1300"].id && Number(l.debitAmount) > 0,
      );
      expect(inventoryLine).toBeDefined();
      expect(Number(inventoryLine.debitAmount)).toBe(500.0);

      // Verify AP credit
      const apLine = entry.lines.find(
        (l) =>
          l.accountId === accounts["2100"].id && Number(l.creditAmount) > 0,
      );
    ...
    it("should calculate total from purchase order item unitCost", async () => {
      ...
      expect(Number(inventoryLine.debitAmount)).toBe(1000.0); // 10 * 100
    });

    it("should update product baseCost with new weighted average", async () => {
      // Product has baseCost 50, quantity 1000
      expect(Number(testProduct.baseCost)).toBe(50.0);
  ...
  describe("Vendor Payment Auto-Posting", () => {
    it("should auto-post journal entry when vendor payment made", async () => {
      ...
      // Verify AP debit
      const apLine = entry.lines.find(
        (l) => l.accountId === accounts["2100"].id && Number(l.debitAmount) > 0,
      );
      expect(apLine).toBeDefined();
      expect(Number(apLine.debitAmount)).toBe(1000.0);

      // Verify Cash credit
      const cashLine = entry.lines.find(
        (l) =>
          l.accountId === accounts["1100"].id && Number(l.creditAmount) > 0,
      );
      expect(cashLine).toBeDefined();
      expect(Number(cashLine.creditAmount)).toBe(1000.0);
    });
    ...
    it("should reduce AP balance correctly", async () => {
      ...
      const paymentApLine = paymentEntries[0].lines.find(
        (l) => l.accountId === accounts["2100"].id,
      );
      expect(Number(paymentApLine.debitAmount)).toBe(1000.0);

      // Net AP balance should be 0
      const allApLines = await journalEntryLineRepo.find({
        where: { accountId: accounts["2100"].id },
      });
      const totalDebit = allApLines.reduce(
        (sum, line) => sum + Number(line.debitAmount),
        0,
      );
      const totalCredit = allApLines.reduce(
    ...
    it("should handle partial payments", async () => {
      ...
      const payment2 = (await vendorPaymentRepo.save(
        payment2Data,
  ...
  describe("Stock Adjustment Auto-Posting", () => {
    it("should auto-post journal entry for inventory increase", async () => {
      ...
      expect(inventoryLine).toBeDefined();
      expect(Number(inventoryLine.debitAmount)).toBe(1000.0); // 20 * 50

      // Verify Adjustment Gain credit
      const gainLine = entry.lines.find(
        (l) =>
          l.accountId === accounts["7100"].id && Number(l.creditAmount) > 0,
      );
      expect(gainLine).toBeDefined();
      expect(Number(gainLine.creditAmount)).toBe(1000.0);
    });
    ...
    it("should auto-post journal entry for inventory decrease", async () => {
      ...
      const lossLine = entry.lines.find(
        (l) => l.accountId === accounts["6100"].id && Number(l.debitAmount) > 0,
      );
      expect(lossLine).toBeDefined();
      expect(Number(lossLine.debitAmount)).toBe(1000.0); // 20 * 50
      ...
      expect(inventoryLine).toBeDefined();
      expect(Number(inventoryLine.creditAmount)).toBe(1000.0);
    });
    ...
    it("should handle mixed increases and decreases", async () => {
      ...
      const savedProduct2 = (await productRepo.save(
        product2,
      ...
      const inventoryDebit = entry.lines.find(
        (l) =>
          l.accountId === accounts["1300"].id &&
          Number(l.debitAmount) === 500.0,
      );
      const gainCredit = entry.lines.find(
        (l) =>
          l.accountId === accounts["7100"].id &&
          Number(l.creditAmount) === 500.0,
      );
      expect(inventoryDebit).toBeDefined();
      expect(gainCredit).toBeDefined();

      // Verify decrease lines (10 * 75 = 750)
      const lossDebit = entry.lines.find(
        (l) =>
          l.accountId === accounts["6100"].id &&
          Number(l.debitAmount) === 750.0,
      );
      const inventoryCredit = entry.lines.find(
        (l) =>
          l.accountId === accounts["1300"].id &&
          Number(l.creditAmount) === 750.0,
      );
    ...
    it("should calculate values using product baseCost", async () => {
        ...
        items: [
          {
            ...
            newQuantity: 1010,
            difference: 10,
            unitCost: 25.0, // Different from baseCost
          },
        ],
      });
      ...
      expect(Number(inventoryLine.debitAmount)).toBe(250.0); // 10 * 25
    });
  });
