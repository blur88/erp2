...
beforeAll(async () => {
  ...
  await dataSource.query('TRUNCATE TABLE price_lists RESTART IDENTITY CASCADE');
  ...
  const login = await request(app.getHttpServer())
    .post('/auth/login').send({ usernameOrEmail: 'admin', password: 'Admin@123!' }).expect(200);
  accessToken = login.body.accessToken;

  const cat = await request(app.getHttpServer())
    .post('/inventory/categories').set(auth())
    .send({ name: `cal-dates-${Date.now()}` }).expect(201);
  const prod = await request(app.getHttpServer())
    .post('/inventory/products').set(auth())
    .send({ name: 'Cal Date Product', categoryId: cat.body.id, baseCost: 100, stockQuantity: 0 })
    .expect(201);
...
afterAll(async () => {
  if (dataSource?.isInitialized) {
    ...
    if (createdRegional) {
      ...
      const active = await rsRepo.findOne({ where: { isActive: true } });
      if (active) { active.timezone = savedRegionalTz; await rsRepo.save(active); }
    }
  }
  await app?.close();
});
...
describe('calendar-date round-trip (DataSource-backed)', () => {
  it('POST/GET stock adjustment preserves YYYY-MM-DD adjustmentDate string', async () => {
    const res = await request(app.getHttpServer())
      .post('/inventory/stock-adjustments').set(auth())
      .send({ adjustmentDate: '2026-07-20', items: [{ productId, oldQuantity: 0, newQuantity: 1, difference: 1 }] })
      .expect(201);
    expect(res.body.adjustmentDate).toBe('2026-07-20');

    const getRes = await request(app.getHttpServer())
      .get(`/inventory/stock-adjustments/${res.body.id}`).set(auth())
      .expect(200);
  ...
  it('POST/GET price list preserves YYYY-MM-DD effectiveFrom string', async () => {
    const res = await request(app.getHttpServer())
      .post('/price-lists').set(auth())
      .send({ code: `RT-${Date.now()}`, name: 'Round Trip', effectiveFrom: '2026-07-20' })
      .expect(201);
    expect(res.body.effectiveFrom).toBe('2026-07-20');

    const getRes = await request(app.getHttpServer())
      .get(`/price-lists/${res.body.id}`).set(auth())
      .expect(200);
...
describe('calendar-date effective-list boundary (app-tz)', () => {
  ...
  it('returns a price list effective through today (app-tz, last-day inclusive)', async () => {
    ...
    await request(app.getHttpServer()).post('/price-lists').set(auth())
      .send({ code: SEEDED_CODE, name: 'Effective today', effectiveTo: '2026-07-20' }).expect(201);

    const res = await request(app.getHttpServer())
      .get('/price-lists/effective').set(auth()).expect(200);
  ...
  it('filters stock adjustments inclusively on both date bounds', async () => {
    ...
    for (const d of ['2026-06-30', '2026-07-01', '2026-07-31', '2026-08-01']) {
      const r = await request(app.getHttpServer())
        .post('/inventory/stock-adjustments').set(auth())
        .send({ adjustmentDate: d, items: [{ productId, oldQuantity: 0, newQuantity: 1, difference: 1 }] })
        .expect(201);
      ids[d] = r.body.id;
    }
    const res = await request(app.getHttpServer())
      .get('/inventory/stock-adjustments?fromDate=2026-07-01&toDate=2026-07-31').set(auth()).expect(200);
