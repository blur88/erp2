...
- **Every gate asserts a positive test count.** Both ESM failure modes — a suite that fails to load, and a whole run under the wrong module mode — report **`Tests: 0 total`**, which reads as green. `npm run test` must report 1654 passing / 127 suites, `npm run test:redis` exactly 11 passing, `npm run test:e2e` a positive count with zero failures.
