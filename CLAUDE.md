...
jsdom has no layout engine and does not evaluate `@media print`, so **the Vitest suite cannot validate this path** — it can only assert that the marker class and the targeted selectors exist. That is a limit of jsdom, not of automation: a headless browser does cover it, and `frontend/e2e/accounting-print.spec.ts` (CI job `Accounting Reports - Print/PDF Gate`, #1214) asserts print-media visibility, print-visible text equality, render order, a multi-page PDF, and an ancestor height/overflow clamp scan. Until that job is added to ruleset 15609777's required checks it does not block a merge, so a **multi-page browser print preview remains a required gate** for any change to these files. `ci.yml` triggers on `pull_request` and `workflow_dispatch` only — there is deliberately no `push` trigger, so **merging never produces a run on `main`**; a main run must be dispatched by hand (`gh workflow run ci.yml --ref main`, #1217), and only such a run can satisfy the second half of BS-P3's downgrade condition in `docs/modules/accounting/BALANCE_SHEET_QA.md`. This failed twice undetected during #1172: the report tree was `display: none` while two rounds of overflow fixes were applied downstream of it.
