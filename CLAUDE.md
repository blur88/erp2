...
**Never restore unconditional `#root` hiding** — it prints a blank sheet for every non-portaled report. A non-portaled print page must also release **every** height/overflow constraint on the ancestor chain (`100vh`, `height: 100%`, `overflow: auto`), or the report is pinned to a single viewport and silently truncates. `accountingReportPrint.css` releases `#root`, its children, `main`, `.app-shell-root` (`MainLayout`'s outer 100vh flex Box — it sits one level *below* `#root > *` and above `main`, so no structural selector reaches it), and `EntityTable`'s own `.entity-table-card` / `-frame` / `-scroller`, which are the real overflow owners. Those class names are print hooks, not style hooks: renaming one truncates every printed report.

jsdom has no layout engine and does not evaluate `@media print`, so **the Vitest suite cannot validate this path** — it can only assert that the marker class and the targeted selectors exist. That is a limit of jsdom, not of automation: a headless browser does cover it, and `frontend/e2e/accounting-print.spec.ts` (CI job `Accounting Reports - Print/PDF Gate`, #1214) asserts print-media visibility, print-visible text equality, render order, a multi-page PDF, and an ancestor height/overflow clamp scan. That job **is** a required check on ruleset 15609777 (added 2026-09-10, #1217), so a change that breaks printing cannot merge; the manual multi-page browser preview was downgraded to a spot-check at the same time. It is still worth running by hand when a change alters print layout, adds rows or sections, or touches the print stylesheet — the gate asserts those properties against its own Balance Sheet fixture, not against a report whose shape you just changed. See `docs/modules/accounting/BALANCE_SHEET_QA.md` (BS-P3) for the procedure and the residual caveats. `ci.yml` triggers on `pull_request` and `workflow_dispatch` only — there is deliberately no `push` trigger, so **merging never produces a run on `main`**; validating main takes a deliberate `gh workflow run ci.yml --ref main`. This failed twice undetected during #1172: the report tree was `display: none` while two rounds of overflow fixes were applied downstream of it.
