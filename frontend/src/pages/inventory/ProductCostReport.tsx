...
const ProductCostReport: React.FC = () => {
  ...
  const [reportData, setReportData] = useState<ProductCostItem[]>([])
    ...
    sortedData.forEach((row, idx) => {
      ...
      selectedColumns.forEach(col => {
        ...
        if (col === 'productName' || col === 'categoryName' || col === 'transactionType' || col === 'orderNumber') {
          ...
          displayValue = ((row as any)[col] || 0).toFixed(2)
  ...
  const getSortedData = () => {
    ...
    if (groupBy !== 'none') {
      filtered.sort((a, b) => {
        // Handle special grouping for category and product
        if (groupBy === 'categoryAndProduct') {
          const catResult = compareValues(a, b, 'categoryName')
          if (catResult !== 0) return catResult
          const prodResult = compareValues(a, b, 'productName')
          if (prodResult !== 0) return prodResult
        } else {
                ...
                <Stack spacing={2}>
                  <FormControl fullWidth size="small" sx={{ '& .MuiInputLabel-root': { fontSize: '0.75rem' }, '& .MuiSelect-select': { fontSize: '0.75rem' } }}>
                    ...
                    >
                      ...
                      <MenuItem value="quantityAfter">Qty After</MenuItem>
                      <MenuItem value="costChange">Cost Change</MenuItem>
                  ...
                  <FormControl fullWidth size="small" sx={{ '& .MuiInputLabel-root': { fontSize: '0.75rem' }, '& .MuiSelect-select': { fontSize: '0.75rem' } }}>
                    ...
                    >
                      ...
                      <MenuItem value="categoryName">Category</MenuItem>
                      <MenuItem value="productName">Product</MenuItem>
                      <MenuItem value="categoryAndProduct">Category and Product</MenuItem>
                    </Select>
                  </FormControl>
                  ...
                  {loading ? (
                    ...
                    <>
                      ...
                      <Typography variant="body2" sx={{
                        ...
                        Configure the filters on the left and click "Generate Report" to view the product cost analysis.
                      </Typography>
                    </>
                  )}
                </Box>
                  ...
                  >
                    <TableHead>
                      <TableRow sx={{ '& .MuiTableCell-head': {
                        ...
                        {selectedColumns.includes('quantityAfter') && <TableCell align="center">Qty After</TableCell>}
                        {selectedColumns.includes('costChange') && <TableCell align="center">Cost Change</TableCell>}
                      ...
                      {(() => {
                        ...
                        paginatedData.forEach((row, idx) => {
                          ...
                          const getGroupLabel = (field: string, r: any) => {
                            ...
                            } else if (field === 'categoryAndProduct') {
                              return `${r.categoryName} - ${r.productName}`
                            }
                            return r[field]
                          }
                          ...
                          rows.push(
                            ...
                            >
                              ...
                              {selectedColumns.includes('quantityChange') && (
                                <TableCell align="right" sx={{ fontSize: '0.8rem', color: row.quantityChange >= 0 ? 'success.main' : 'error.main' }}>
                                  {row.quantityChange >= 0 ? '+' : ''}{row.quantityChange.toFixed(2)}
                                </TableCell>
                              )}
                              {selectedColumns.includes('quantityAfter') && (
                                <TableCell align="right" sx={{ fontSize: '0.8rem' }}>
                                  {row.quantityAfter.toFixed(2)}
                                </TableCell>
                              )}
