...
const PriceListReport: React.FC = () => {
  ...
  const [selectedColumns, setSelectedColumns] = useState<string[]>([
    'productName', 'categoryName', 'price', 'discountedPrice', 'salesCost'
  ])
    ...
    sortedData.forEach((row) => {
      ...
      selectedColumns.forEach(col => {
        ...
        if (col === 'productName' || col === 'categoryName') {
          ...
          displayValue = formatCurrency((row as any)[col] || 0)
  ...
  const getSortedData = () => {
    ...
    const compareValues = (a: any, b: any, field: string) => {
      ...
      // For numeric fields, sort descending (largest first)
      if (field === 'price' || field === 'discountedPrice' || field === 'salesCost') {
                ...
                <Stack spacing={2}>
                  <FormControl fullWidth size="small" sx={{ '& .MuiInputLabel-root': { fontSize: '0.75rem' }, '& .MuiSelect-select': { fontSize: '0.75rem' } }}>
                    ...
                    <Select
                      ...
                      onChange={(e) => {
                        ...
                        if (value.includes('all')) {
                          const allColumns = ['productName', 'categoryName', 'price', 'discountedPrice', 'salesCost']
                    ...
                    >
                      ...
                      <MenuItem value="discountedPrice">Discounted Price</MenuItem>
                      <MenuItem value="salesCost">Sales Cost</MenuItem>
                    </Select>
                  </FormControl>
                  ...
                  {loading ? (
                    ...
                    <>
                      ...
                      <Typography variant="body2" color="text.secondary">
                        Configure the filters on the left and click "Generate Report" to view the product price list.
                      </Typography>
                    </>
                  )}
                </Box>
                  ...
                  >
                    <TableHead>
                      <TableRow sx={{ '& .MuiTableCell-head': {
                        ...
                        {selectedColumns.includes('price') && <TableCell align="center">Price</TableCell>}
                        {selectedColumns.includes('discountedPrice') && <TableCell align="center">Discounted Price</TableCell>}
                        {selectedColumns.includes('salesCost') && <TableCell align="center">Sales Cost</TableCell>}
                      </TableRow>
                    </TableHead>
                    <TableBody>
                      {paginatedData.map((row, idx) => {
                        ...
                        const calculateGroupSubtotals = () => {
                          ...
                          return {
                            price: groupData.reduce((sum, r) => sum + r.price, 0),
                            discountedPrice: groupData.reduce((sum, r) => sum + r.discountedPrice, 0),
                            salesCost: groupData.reduce((sum, r) => sum + r.salesCost, 0),
                          }
                        }
                        ...
                        return (
                          <React.Fragment key={idx}>
                            ...
                            >
                              ...
                              {selectedColumns.includes('discountedPrice') && (
                                <TableCell align="right" sx={{ fontSize: '0.8rem' }}>
                                  {formatCurrency(row.discountedPrice)}
                                </TableCell>
                              )}
                              {selectedColumns.includes('salesCost') && (
                                <TableCell align="right" sx={{ fontSize: '0.8rem' }}>
                                  {formatCurrency(row.salesCost)}
                                </TableCell>
                              )}
                            </TableRow>
                                ...
                                <TableRow sx={{
                                  ...
                                  {selectedColumns.includes('price') && (
                                    <TableCell align="right" sx={{ fontWeight: 700 }}>
                                      {formatCurrency(groupSubtotals.price)}
                                    </TableCell>
                                  )}
                                  {selectedColumns.includes('discountedPrice') && (
                                    <TableCell align="right" sx={{ fontWeight: 700 }}>
                                      {formatCurrency(groupSubtotals.discountedPrice)}
                                    </TableCell>
                                  )}
                                  {selectedColumns.includes('salesCost') && (
                                    <TableCell align="right" sx={{ fontWeight: 700 }}>
                                      {formatCurrency(groupSubtotals.salesCost)}
                                    </TableCell>
                                  )}
                                </TableRow>
                      ...
                      {totals && (
                        ...
                        >
                          ...
                          {selectedColumns.includes('salesCost') && (
                            <TableCell align="right" sx={{ fontWeight: 800 }}>
                              {formatCurrency(totals.salesCost)}
                            </TableCell>
                          )}
                        </TableRow>
                      )}
