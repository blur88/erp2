...
const MovementSummaryReport: React.FC = () => {
  ...
  const [selectedColumns, setSelectedColumns] = useState<string[]>([
    'productName', 'categoryName', 'quantityIn', 'quantityOut', 'quantityOnHand'
  ])
  ...
  const handleClearFilters = () => {
    ...
    setEndDate('')
    setReportData([])
    setSelectedColumns(['productName', 'categoryName', 'quantityIn', 'quantityOut', 'quantityOnHand'])
    setGroupBy('none')
    ...
    sortedData.forEach((row) => {
      ...
      selectedColumns.forEach(col => {
        ...
        if (col === 'productName' || col === 'categoryName') {
          ...
          displayValue = Math.round((row as any)[col]).toString()
  ...
  const getSortedData = () => {
    ...
    const compareValues = (a: any, b: any, field: string) => {
      ...
      if (field === 'quantityIn' || field === 'quantityOut' || field === 'quantityOnHand') {
                ...
                <Stack spacing={2}>
                  <FormControl fullWidth size="small" sx={{ '& .MuiInputLabel-root': { fontSize: '0.75rem' }, '& .MuiSelect-select': { fontSize: '0.75rem' } }}>
                    ...
                    <Select
                      ...
                      onChange={(e) => {
                        ...
                        if (value.includes('all')) {
                          const allColumns = ['productName', 'categoryName', 'quantityIn', 'quantityOut', 'quantityOnHand']
                    ...
                    >
                      ...
                      <MenuItem value="quantityIn">Qty In</MenuItem>
                      <MenuItem value="quantityOut">Qty Out</MenuItem>
                      <MenuItem value="quantityOnHand">Qty on Hand</MenuItem>
                    </Select>
                  </FormControl>
                  ...
                  >
                    <TableHead>
                      <TableRow sx={{ '& .MuiTableCell-head': {
                        ...
                        {selectedColumns.includes('productName') && <TableCell align="center">Item</TableCell>}
                        {selectedColumns.includes('categoryName') && <TableCell align="center">Category</TableCell>}
                        {selectedColumns.includes('quantityIn') && <TableCell align="center">Qty In</TableCell>}
                        {selectedColumns.includes('quantityOut') && <TableCell align="center">Qty Out</TableCell>}
                        {selectedColumns.includes('quantityOnHand') && <TableCell align="center">Qty on Hand</TableCell>}
                      </TableRow>
                    </TableHead>
                    <TableBody>
                            ...
                            >
                              ...
                              {selectedColumns.includes('quantityIn') && (
                                <TableCell align="right" sx={{ fontSize: '0.8rem' }}>
                                  {Math.round(row.quantityIn)}
                                </TableCell>
                              )}
                              {selectedColumns.includes('quantityOut') && (
                                <TableCell align="right" sx={{ fontSize: '0.8rem' }}>
                                  {Math.round(row.quantityOut)}
                                </TableCell>
                              )}
                              {selectedColumns.includes('quantityOnHand') && (
                                <TableCell align="right" sx={{ fontSize: '0.8rem' }}>
                                  {Math.round(row.quantityOnHand)}
                                </TableCell>
                              )}
                            </TableRow>
